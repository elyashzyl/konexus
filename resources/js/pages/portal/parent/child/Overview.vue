...
import { ArrowLeft, ArrowRight, BookOpen, CalendarDays, CheckCircle2, FileText, GraduationCap, MapPin, ShieldCheck, Sparkles, UserRound, type LucideIcon } from 'lucide-vue-next';
...
onMounted(async () => {
    try {
        profile.value = await portalApi.parent.child(childId.value);
    } catch (error) {
...
const quickLinks = computed<{ title: string; description: string; href: string; icon: LucideIcon }[]>(() => [
    { title: 'Grades', description: 'Published grading records and report cards.', href: `/portal/parent/children/${childId.value}/grades`, icon: BookOpen },
    { title: 'Weekly schedule', description: 'The timetable for the current term.', href: `/portal/parent/children/${childId.value}/schedule`, icon: CalendarDays },
    { title: 'Enrollment history', description: 'Every school year on the official record.', href: `/portal/parent/children/${childId.value}/enrollments`, icon: GraduationCap },
    { title: 'Private documents', description: 'Secure files released by the registrar.', href: `/portal/parent/children/${childId.value}/documents`, icon: FileText },
]);
...
<template>
    <div class="relative">
        ...
        <div class="relative w-full px-5 pb-20 sm:px-8 lg:px-12">
            <RouterLink
                to="/portal/parent"
                class="portal-rise inline-flex items-center gap-2 px-2 pt-10 text-sm font-medium text-primary hover:underline"
            >
            ...
            <div v-else-if="!profile" class="pt-10">
                <PortalEmptyState
                    ...
                    description="The record may have been removed or is not linked to your account."
                />
            </div>

            <template v-else>
                <section class="portal-rise grid gap-12 pt-10 pb-14 lg:grid-cols-[1.25fr_0.75fr] lg:gap-16">
                    <div class="flex flex-col justify-between">
                        <div>
                            ...
                            <p class="mt-5 text-[15px] leading-7 text-muted-foreground">
                                Their records are kept here as the registrar maintains them — private, current, and always available to you.
                            </p>
                        </div>
                ...
                <section class="portal-rise mt-16" style="animation-delay: 160ms">
                    <div class="flex items-end justify-between gap-6">
                        <div>
                            ...
                            <h2 class="mt-3 font-display text-3xl font-medium tracking-[-0.015em] text-foreground">Their portal</h2>
                        </div>
