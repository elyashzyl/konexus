...
onMounted(async () => {
    try {
        schedule.value = await portalApi.teacher.schedule();
    } catch (error) {
...
<template>
    <main class="w-full px-5 pt-10 pb-20 sm:px-8 lg:px-12 lg:pt-16">
        <PortalPageHeader
            ...
            description="Your timetable for the current term, as published by the registrar."
        >
            <template #actions>
        ...
        <div v-else-if="!schedule.length" class="mt-12">
            <PortalEmptyState
                ...
                description="Your class schedule will appear here once it has been published."
            />
        </div>

        <WeeklyScheduleGrid v-else class="portal-rise mt-12" :entries="schedule" detail="room" />
    </main>
</template>
