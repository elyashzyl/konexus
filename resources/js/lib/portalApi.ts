...
export const portalApi = {
    student: {
        dashboard: () =>
            api
                .get<{
                    data: { profile: ChildSummary | null; announcements: AnnouncementItem[]; modules: Record<string, boolean> };
        ...
        attendance: () => api.get<{ data: { summary: Partial<AttendanceSummary> } }>('/portal/student/attendance').then((r) => r.data.data.summary),
    ...
    parent: {
        dashboard: () =>
            api
                .get<{
                    data: {
                        parent: { name: string; email: string; contact_number: string } | null;
        ...
        schedule: (id: number) =>
            api.get<{ data: { items: PortalScheduleEntry[] } }>(`/portal/parent/children/${id}/schedule`).then((r) => r.data.data.items),
        grades: (id: number) => api.get<{ data: AcademicSummary }>(`/portal/parent/children/${id}/grades`).then((r) => r.data.data),
        enrollments: (id: number) =>
            api.get<{ data: { items: PortalEnrollment[] } }>(`/portal/parent/children/${id}/enrollments`).then((r) => r.data.data.items),
        documents: (id: number) =>
            api.get<{ data: { items: PortalDocument[] } }>(`/portal/parent/children/${id}/documents`).then((r) => r.data.data.items),
    },
    teacher: {
        dashboard: () =>
            api
        ...
        advisoryClass: () =>
            api
                .get<{
                    data: { class: { id: number; name: string; section: string | null; students: AdvisoryStudent[] } | null };
                }>('/portal/teacher/advisory-class')
                .then((r) => r.data.data.class),
